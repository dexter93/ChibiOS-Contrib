...
#define SPI_DMAHTIE_DIS					0		//[4:4]DMA half transfer interrupt enable bit (SPI0 only control bit)
#define SPI_DMAHTIE_EN					1
#define mskSPI_DMAHTIE_DIS				(SPI_DMAHTIE_DIS << 4)
#define mskSPI_DMAHTIE_EN				(SPI_DMAHTIE_EN << 4)

#define SPI_DMATCIE_DIS					0		//[5:5]DMA transfer complete interrupt enable bit (SPI0 only control bit)
#define SPI_DMATCIE_EN					1
#define	mskSPI_SPI_DMATCIE_DIS			(SPI_DMATCIE_DIS << 5)
#define mskSPI_SPI_DMATCIE_EN			(SPI_DMATCIE_EN << 5)
...
#define mskSPI_DMAHTIF					(0x1 << 4)	//[4:4]DMA half transfer flag (SPI0 only control bit)
#define mskSPI_DMAHTIC					mskSPI_DMAHTIF

#define mskSPI_DMATCIF					(0x1 << 5)	//[5:5]DMA transfer complete flag (SPI0 only control bit)
#define mskSPI_DMATCIC					mskSPI_DMATCIF
...
/* SPI n DMA Control register <SPIn_DMACTRL> (0x24) (SPI0 ONLY) */
												//[0:0]SPI to SPI DMA enable bit
#define SPI_DMAEN_DIS					0		//Disable
#define SPI_DMAEN_EN					1		//Enable
#define mskSPI_SPI_DMAEN_DIS			(SPI_DMAEN_DIS << 0)
#define mskSPI_SPI_DMAEN_EN				(SPI_DMAEN_EN << 0)

												//[1:1]SPI data transfer direction
#define SPI_DIR_MODE1					0		//SPI1_DATA(RX) to SPI0_DATA(TX), when DMATCIE and DMAHTIE enable, and trigger SPI0 interrupt
#define SPI_DIR_MODE2					1		//SPI0_DATA(RX) to SPI1_DATA(TX) , when DMATCIE and DMAHTIE enable, and trigger SPI1 interrupt
#define mskSPI_DIR_MODE1				(SPI_DIR_MODE1 << 1)
#define mslSPI_DIR_MODE2				(SPI_DIR_MODE2 << 1)
